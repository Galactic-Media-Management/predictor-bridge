  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const retryable =
        ...
        message.includes('Unable to locate ContractCode at') ||
...
async function verifyProxyOnEtherscan(networkName, proxyAddress) {
  ...
  if (submitJson.status !== '1') {
    ...
    if (
      ...
      msg.toLowerCase().includes('proxy contract is already verified')
    ) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (
      result.toLowerCase().includes('does not seem to be verified') ||
      result.toLowerCase().includes('pending in queue')
    ) {
      continue;
    }
